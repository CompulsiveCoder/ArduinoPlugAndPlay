echo "Updating plug and play with auto update..."
...
wget -q --no-cache -O - https://raw.githubusercontent.com/CompulsiveCoder/ArduinoPlugAndPlay/$BRANCH/scripts-web/init-from-web.sh | bash -s $BRANCH || exit 1

echo "Reinstalling the service file (by downloading install-service-from-web.sh file)..."

wget -q --no-cache -O - https://raw.githubusercontent.com/CompulsiveCoder/ArduinoPlugAndPlay/$BRANCH/scripts-web/install-service-from-web.sh | bash -s $BRANCH $INSTALL_DIR || exit 1

echo "Restarting service..."
sh systemctl.sh restart arduino-plug-and-play.service || exit 1

echo "Finished setting up plug and play with auto update"

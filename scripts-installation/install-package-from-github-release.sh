...
echo "Installing package $PACKAGE_NAME $PACKAGE_VERSION$POSTFIX..."
...
#TODO: Fix or remove
  ...
  # If the package still isn't found
    ...
    # Download the package from the web    
  	curl -O https://github.com/$PACKAGE_OWNER/$PACKAGE_NAME/releases/download/v$PACKAGE_VERSION$POSTFIX/$PACKAGE_NAME.$PACKAGE_VERSION$POSTFIX.zip || exit 1
